...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="{{ url('/display-positions') }}" class="btn btn-primary">View Active Positions</a>
    </div>
    ...
    <form action="{{ url('/display-archived-positions') }}" method="GET" class="mb-3">
        <div class="input-group">
            <input type="search" name="search" id="" class="form-control" placeholder="Search archived position..." autocomplete="off">
        ...
        <tbody>
            @foreach ($positions as $position)
                <tr>
                    ...
                    <td>{{ $position->deleted_at->format('M d, Y h:i A') }}</td>
                    <td>
                        @if(strtolower($electionStatus ?? 'pending') === 'pending')
                            <form action="{{ url('restore/'.$position->position_id) }}" method="POST" class="d-inline">
                                @csrf
                                <button type="submit" class="btn btn-sm btn-success mb-1" onclick="return confirm('Are you sure you want to restore this position?')">Restore</button>
                            </form>
                            <form action="{{ url('force-delete-position/'.$position->position_id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-danger mb-1" onclick="return confirm('Are you sure you want to permanently delete this position? This action cannot be undone!')">Delete</button>
                            </form>
                        @else
