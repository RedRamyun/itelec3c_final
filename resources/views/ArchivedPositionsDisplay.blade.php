...
<head>
    ...
    <title>Archived Positions</title>
                ...
                <div class="col-md-11 col-lg-10">
                    <div class="position-container">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <h1 class="mb-0">Archived Positions</h1>
                            <div class="d-flex gap-2">
                                <a href="{{ url('/display-positions') }}" class="btn btn-outline-secondary">
                                    <i class="fas fa-arrow-left me-2"></i> View Active Positions
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <form action="{{ url('/display-archived-positions') }}" method="GET" class="flex-grow-1 me-3">
                                <div class="input-group">
                                    <input type="search" name="search" class="form-control" placeholder="Search archived position..." value="{{ request('search') }}" autocomplete="off">
                                    ...
                                    <tbody>
                                        @foreach ($positions as $position)
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="position-badge">{{ $position->position_name }}</span>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <span class="deleted-badge">
                                                        {{ $position->deleted_at->format('M d, Y h:i A') }}
                                                    </span>
                                                </td>
                                                <td>
                                                    @if(strtolower($electionStatus ?? 'pending') === 'pending')
                                                        <form action="{{ url('restore/'.$position->position_id) }}" method="POST" class="d-inline">
                                                            @csrf
                                                            <button type="submit" class="btn btn-sm btn-success btn-action" onclick="return confirm('Are you sure you want to restore this position?')">
                                                                <i class="fas fa-undo me-1"></i>Restore
                                                            </button>
                                                        </form>
                                                        <form action="{{ url('force-delete-position/'.$position->position_id) }}" method="POST" class="d-inline">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="btn btn-sm btn-danger btn-action" onclick="return confirm('Are you sure you want to permanently delete this position? This action cannot be undone!')">
                        ...
                        @else
                            <div class="empty-state">
                                <div class="mb-3">📋</div>
                                <h4>No Archived Positions Found</h4>
                                <p class="mb-4">There are no archived positions to display.</p>
                            </div>
                        @endif
                    </div>
                </div>
