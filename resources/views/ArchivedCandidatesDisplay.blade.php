...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="{{ url('/display-candidates') }}" class="btn btn-primary">View Active Candidates</a>
    </div>
    ...
    <form action="{{ url('/display-archived-candidates') }}" method="GET" class="mb-3">
        <div class="input-group">
            <input type="search" name="search" class="form-control" placeholder="Search archived candidate..." autocomplete="off">
        ...
        <tbody>
            @foreach ($candidates as $candidate)
                <tr>
                    ...
                    <td>{{ $candidate->deleted_at->format('M d, Y h:i A') }}</td>
                    <td>
                        @if(strtolower($electionStatus ?? 'pending') === 'pending')
                            <form action="{{ url('restore-candidate/'.$candidate->candidate_id) }}" method="POST" class="d-inline">
                                @csrf
                                <button type="submit" class="btn btn-sm btn-success mb-1" onclick="return confirm('Are you sure you want to restore this candidate?')">Restore</button>
                            </form>
                            <form action="{{ url('force-delete-candidate/'.$candidate->candidate_id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-danger mb-1" onclick="return confirm('Are you sure you want to PERMANENTLY delete this candidate? This action cannot be undone!')">Delete</button>
                            </form>
                        @else
                            <button class="btn btn-sm btn-success mb-1" disabled title="Election must be in Pending status">Restore</button>
                            <button class="btn btn-sm btn-danger mb-1" disabled title="Election must be in Pending status">Delete</button>
                        @endif
                    </td>
                </tr>
            @endforeach
