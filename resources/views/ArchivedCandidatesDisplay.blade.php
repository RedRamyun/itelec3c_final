                    ...
                    <div class="position-container">
                        ...
                        @if(strtolower($electionStatus ?? 'pending') !== 'pending')
                            <div class="election-status-alert">
                                ...
                                <strong>Actions Disabled:</strong> Candidate management is locked because the election status is currently "<strong>{{ ucfirst($electionStatus) }}</strong>". 
                                Only viewing is available. To modify candidates, the election must be in "Pending" status.
                            </div>
                        @endif

                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <h1 class="mb-0">Archived Candidates</h1>
                            <div class="d-flex gap-2">
                                <a href="{{ url('/display-candidates') }}" class="btn btn-outline-secondary">
                                    <i class="fas fa-arrow-left me-2"></i> View Active Candidates
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <form action="{{ url('/display-archived-candidates') }}" method="GET" class="flex-grow-1 me-3">
                                <div class="input-group">
                                    <input type="search" name="search" class="form-control" placeholder="Search archived candidate..." value="{{ request('search') }}" autocomplete="off">
                                    ...
                                    <tbody>
                                        @foreach ($candidates as $candidate)
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="candidate-badge">{{ $candidate->candidate_name }}</span>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <span class="deleted-badge">
                                                        {{ $candidate->deleted_at->format('M d, Y h:i A') }}
                                                    </span>
                                                </td>
                                                <td>
                                                    @if(strtolower($electionStatus ?? 'pending') === 'pending')
                                                        <form action="{{ url('restore-candidate/'.$candidate->candidate_id) }}" method="POST" class="d-inline">
                                                            @csrf
                                                            <button type="submit" class="btn btn-sm btn-success btn-action" onclick="return confirm('Are you sure you want to restore this candidate?')">
                                                                <i class="fas fa-undo me-1"></i>Restore
                                                            </button>
                                                        </form>
                                                        <form action="{{ url('force-delete-candidate/'.$candidate->candidate_id) }}" method="POST" class="d-inline">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="btn btn-sm btn-danger btn-action" onclick="return confirm('Are you sure you want to PERMANENTLY delete this candidate? This action cannot be undone!')">
                        ...
                        @else
                            <div class="empty-state">
                                <div class="mb-3">📋</div>
                                <h4>No Archived Candidates Found</h4>
                                <p class="mb-4">There are no archived candidates to display.</p>
                            </div>
                        @endif
                    </div>
                </div>
