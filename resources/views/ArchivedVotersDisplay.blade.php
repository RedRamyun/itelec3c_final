...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="{{ url('/voters') }}" class="btn btn-primary">View Active Voters</a>
    </div>
    ...
    <form action="{{ url('/display-archived-voters') }}" method="GET" class="mb-3">
        <div class="input-group">
            <input type="search" name="search" class="form-control" placeholder="Search archived voter..." autocomplete="off">
                ...
                <tr>
                    ...
                    <td>
                        <form action="{{ url('restore-voter/'.$voter->voter_id) }}" method="POST" class="d-inline">
                            @csrf
                            <button type="submit" class="btn btn-sm btn-success mb-1" onclick="return confirm('Are you sure you want to restore this voter?')">Restore</button>
                        </form>
                        <form action="{{ url('force-delete-voter/'.$voter->voter_id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-sm btn-danger mb-1" onclick="return confirm('Are you sure you want to PERMANENTLY delete this voter? This action cannot be undone!')">Delete</button>
                        </form>
                    </td>
                </tr>
            @endforeach
